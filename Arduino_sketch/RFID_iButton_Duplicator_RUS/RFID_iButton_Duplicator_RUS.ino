...
Изменения только в коде скетча - состав устройства и схема подключения аналогична варианту SibMan54 
(можно спокойно перепрошить старые устройства на этот вариант, все будет работать).

Авторы: МЕХАТРОН DIY, AlexMalov, 2019 https://github.com/AlexMalov/EasyKeyDublicatorRFID_OLED

SibMan54, 2023 https://github.com/SibMan54/Arduino-RFID-iButton-Duplicator

Igor Cheprasov, 2026

Схему сборки и файлы корпуса для 3D-печати смотрите здесь https://github.com/SibMan54/Arduino-RFID-iButton-Duplicator
  
  ---------------------Мои доработки-----------------------------
  За основу взят код SibMan54:
  1. Для OLED-дисплея теперь используется библиотека GyverOLED;
  2. Выводимые на дисплей сообщения переведены на русский язык. Длина сообщений подогнана под разрешение 128x32.
...
GyverOLED<SSD1306_128x32, OLED_BUFFER> myOLED(0x3C);
...
enum emRWType { rwUnknown,
                ...
                T5557,
                EM4305 };  // тип болванки
enum emkeyType { keyUnknown,
                 keyDallas,
                 keyTM2004,
                 keyCyfral,
                 keyMetacom,
                 keyEM_Marine,
                 keyMifare };  // тип оригинального ключа
emkeyType keyType;
enum emMode { md_empty,
              md_read,
              md_write,
              md_threeMode };  // режим работы копировальщика
...
void OLED_printError(String st, bool err = true) {
  ...
  if (err) myOLED.print(F("Ошибка!"));
  else myOLED.print(F("Успешно!"));
  myOLED.setCursorXY(0, 12);
  myOLED.print(st);
  myOLED.update();
}

void showModeSplash(emMode mode) {
  myOLED.clear();
  myOLED.setScale(2);
  
  myOLED.setCursorXY(20, 8);
...
void setup() {
  ...
  myOLED.init();   //инициализируем дисплей
  myOLED.clear();  //Очищаем буфер дисплея.
  myOLED.setCursorXY(0, 0);
  myOLED.invertText(1);
  myOLED.print(F(" Копировальщик ключей "));
  myOLED.invertText(0);
  myOLED.setCursorXY(0, 24);
  myOLED.print(F("Чтение списка ключей..."));
  myOLED.update();
  ...
  if (EEPROM_key_count != 0) {
    ...
    myOLED.setCursorXY(0, 24);
    myOLED.print(F(" Нет ключей в памяти  "));
    myOLED.update();
  }
...
emkeyType getKeyType(byte* buf) {
  ...
  // Mifare (UID 4 байта, остальные нули)
  if (buf[4] == 0 && buf[5] == 0 && buf[6] == 0 && buf[7] == 0) return keyMifare;

  // Dallas / iButton / RW1990 и их клоны
  // 0x01 - стандартный DS1990A
  // 0x11, 0x02, 0x0A, 0x12 - распространенные Family Code болванок RW1990/DS1990
  if (buf[0] == 0x01 || buf[0] == 0x11 || buf[0] == 0x02 || buf[0] == 0x0A || buf[0] == 0x12) {
    return keyDallas;
  }

  // Cyfral (старший ниббл = 1, но не 0x11, т.к. 0x11 мы уже отнесли к Dallas)
  if ((buf[0] >> 4) == 0b0001) return keyCyfral;
  // Metacom (старший ниббл = 2)
...
bool write2iBtn() {
  ...
  if (Check == 8) {  // если коды совпадают, ничего писать не нужно
    ...
    OLED_printError(F("Ключ совпадает"));
    Sd_ErrorBeep();
    digitalWrite(R_Led, HIGH);
    delay(1000);
    return false;
...
bool searchIbutton() {
  ...
  // Поддержка стандартных и клоновых Family Code для Dallas/RW1990
  if (addr[0] == 0x01 || addr[0] == 0x11 || addr[0] == 0x02 || addr[0] == 0x0A || addr[0] == 0x12) {
    keyType = keyDallas;
    if (getRWtype() == TM2004) keyType = keyTM2004;

    // Проверка CRC: обязательна только для оригинальных DS1990A (0x01)
    // Для RW1990-клонов (0x11 и др.) CRC часто не соответствует стандарту 1-Wire,
    // поэтому делаем проверку мягкой (предупреждение без прерывания).
    if (OneWire::crc8(addr, 7) != addr[7]) {
      if (addr[0] == 0x01) {
        // Оригинальный ключ с битым CRC — действительно брак
        Serial.println(F("CRC is not valid! "));
        OLED_printError(F("Некорректный CRC"));
        Sd_ErrorBeep();
        digitalWrite(B_Led, HIGH);
        return false;
      } else {
        // Клон — CRC часто отсутствует, но ключ рабочий
        Serial.println(F("CRC mismatch (clone). Ignoring... "));
      }
    }
    return true;
  }

  // Остальные протоколы (Cyfral/Metacom эвристика)
  switch (addr[0] >> 4) {
    case 1:
    ...
    case 3:
      ...
      keyType = keyUnknown;
      break;
    default: keyType = keyUnknown; break;
  }
  return true;
}
...
void rfidACsetOn() {
  ...
  OCR2A = 126;                                     // Задаем период таймера (125 тактов на период, 125 кГц на выводе 3)
  OCR2B = 63;                                      // Задаем скважность на 50% (62/125)
  ...
  if (rfid.PICC_IsNewCardPresent() && rfid.PICC_ReadCardSerial()) {
    ...
    for (byte i = 0; i < 16; i++) {
      ...
      if (rfid.MIFARE_Write(i, buffer, 16)) {
        ...
        OLED_printError(F("Очистка..."), false);
        Sd_WriteStep();
      } else {
        Serial.println("Failed to clear block " + String(i));
        OLED_printError(F("Очистка не удалась"));
        Sd_ErrorBeep();
        result = false;
      }
    }
    ...
    rfid.PCD_StopCrypto1();
    Sd_ReadOK();
    OLED_printError(F("Карта очищена"), false);
    result = true;
    ...
    OLED_printError(F("Очищен сектор 0"), false);
    Sd_ReadOK();
    result = true;
  }
  statOk = result;
